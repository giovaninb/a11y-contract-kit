...
public struct InteractiveA11yHTMLReporter {
    ...
    private var embeddedJavaScript: String {
        ...
        function buildPatchCommand(selected, multiline) {
          if (!data.reportPath || !data.projectRoot) return '';
          const issueIds = selected.map(item => item.id).join(',');
          const parts = [
            '.build/release/a11y-contract export-fixes patch',
        ...
        document.getElementById('apply-fixes').addEventListener('click', async () => {
          ...
          const exportText = buildExport();
          const patchCommand = buildPatchCommand(selected, true);
          ...
          if (patchCommand) {
            const shellCommand = buildPatchCommand(selected, false);
            downloadText(
              'apply-a11y-patches.sh',
              '#!/bin/bash\\nset -euo pipefail\\ncd "' + data.projectRoot + '"\\n' + shellCommand + '\\n'
            );
          }
    ...
    static func uiStrings() -> [String: [String: String]] {
        [
            "en": [
                ...
                "notice": "Select issues, choose a fix style, then apply. Snippets are copied to the clipboard. To write changes into source files, run the generated <strong>apply-a11y-patches.sh</strong> script or the patch command shown in the preview.",
                "fix_style": "Fix style",
                ...
                "patch_command_header": "# Apply directly to source files:",
                ...
                "toast_applied_patch": "Fixes accepted. Run apply-a11y-patches.sh to update source files.",
                "toast_applied_manual_patch": "Fixes accepted. Downloaded apply-a11y-patches.sh — run it in the project root.",
            ],
            "pt": [
                "title": "Seletor de correções A11y",
                "notice": "Selecione os achados, escolha o estilo e clique em <strong>Corrigir</strong>. Os snippets são copiados. Para gravar nos arquivos, execute o script <strong>apply-a11y-patches.sh</strong> ou o comando patch exibido na prévia.",
                "fix_style": "Estilo de correção",
                "group_by_component": "Agrupar por componente",
                "select_all": "Selecionar todos",
                "clear": "Limpar",
                "copy_fixes": "Copiar selecionados",
                "apply_fixes": "Corrigir",
                "patch_command_header": "# Aplicar direto nos arquivos:",
                ...
                "toast_applied_patch": "Correções aceitas. Execute apply-a11y-patches.sh para atualizar os arquivos.",
                "toast_applied_manual_patch": "Correções aceitas. Baixamos apply-a11y-patches.sh — execute na raiz do projeto.",
            ],
            "es": [
                "title": "Selector de correcciones A11y",
                "notice": "Seleccione hallazgos, elija el estilo y pulse <strong>Corregir</strong>. Los snippets se copian. Para escribir en los archivos, ejecute <strong>apply-a11y-patches.sh</strong> o el comando patch de la vista previa.",
                "fix_style": "Estilo de corrección",
                "group_by_component": "Agrupar por componente",
                "select_all": "Seleccionar todos",
                "clear": "Limpiar",
                "copy_fixes": "Copiar seleccionados",
                "apply_fixes": "Corregir",
                "patch_command_header": "# Aplicar directamente en los archivos:",
                ...
                "toast_applied_patch": "Correcciones aceptadas. Ejecute apply-a11y-patches.sh para actualizar los archivos.",
                "toast_applied_manual_patch": "Correcciones aceptadas. Descargamos apply-a11y-patches.sh — ejecútelo en la raíz del proyecto.",
            ],
        ]
    }
